...
class PresentationChrome
{
    ...
    public function standardEmbed(string $canonicalUrl, string $forumOrigin): string
    {
        $configuration = [
            'discourseUrl' => rtrim($forumOrigin, '/').'/',
        ...
        return $this->styles()
            ...
            .'<div class="discussionbridge-discussion__header"><h2>Discussion</h2></div>'
            .'<div id="discourse-comments"></div>'
            .$this->credit()
            .'</section>'
            .'<script>window.DiscourseEmbed='.json_encode($configuration, JSON_THROW_ON_ERROR | JSON_UNESCAPED_SLASHES).';</script>'
    ...
    public function discourseCredit(bool $enabled): string
    {
        $hidden = $enabled ? '' : ' hidden';
        $wordmark = file_get_contents(dirname(__DIR__, 2).'/resources/assets/discourse-wordmark.svg');
        if (! is_string($wordmark) || $wordmark === '') {
            throw new \RuntimeException('Discourse wordmark asset is unavailable.');
        }

        return '<div data-discussionbridge-attributions><a class="discussionbridge-powered-by" data-discussionbridge-powered-by href="https://www.discourse.org/powered-by" aria-label="Powered by Discourse" rel="nofollow noopener noreferrer"'.$hidden.'><span>Powered by</span><span class="discussionbridge-powered-by__wordmark">'.$wordmark.'</span></a></div>';
    }
